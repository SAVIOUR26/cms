...
(function () {
    ...
    const REST_BASE_FLW = CFG.restBaseFLW || '';
    const REST_BASE_DPO = CFG.restBaseDPO || '';
    const REST_BASE = CFG.restBase || ''; // For editions/subscriptions API
    ...
    async function checkGatewayStatus() {
        try {
            const statusUrl = REST_BASE ? REST_BASE.replace(/\/kanda\/v1$/, '') + '/kanda-pay/v1/gateways/status' : '/wp-json/kanda-pay/v1/gateways/status';
    ...
    // ========== Initialization ==========
    async function init() {
        // CRITICAL: Ensure payment chooser is hidden on load
        if (paymentChooser) {
            paymentChooser.style.display = 'none';
            paymentChooser.hidden = true;
            paymentChooser.setAttribute('aria-hidden', 'true');
